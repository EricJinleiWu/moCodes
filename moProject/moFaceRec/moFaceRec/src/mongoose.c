...
//add by chenruopeng 20170922
#if 1
    conn->httpserver_cb = opts.httpserver_cb;
    conn->websocket_cb = opts.websocket_cb;
#endif
...
//add by chenruopeng 20170922
#if 1
  nc->httpserver_cb = lc->httpserver_cb;
  nc->websocket_cb = lc->websocket_cb;
#endif
...
//add by chenruopeng 20171018
#if 0
	if((unsigned long)nc->user_data == 0)
		continue;
#endif
...
//modified by chenruopeng 20180123
#if 0
  strftime(mod, sizeof(mod), "%d-%b-%Y %H:%M", localtime(&stp->st_mtime));
#else
  struct tm timeLocal;
  struct tm* pLocal = localtime_r(&stp->st_mtime, &timeLocal);
  strftime(mod, sizeof(mod), "%d-%b-%Y %H:%M", pLocal);
#endif
