...
static MOUTILS_INI_SECTION_INFO_NODE * InitSecInfoList(void)
{
    MOUTILS_INI_SECTION_INFO_NODE *pSecInfoHeadNode = NULL;
	pSecInfoHeadNode = (MOUTILS_INI_SECTION_INFO_NODE *)malloc(sizeof(MOUTILS_INI_SECTION_INFO_NODE));
	if(NULL == pSecInfoHeadNode)
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, "Malloc for pSecInfoHeadNode failed!\n");
		return NULL;
	}
	pSecInfoHeadNode->pHeadAttrNode = (MOUTILS_INI_ATTR_INFO_NODE *)malloc(sizeof(MOUTILS_INI_ATTR_INFO_NODE));
	if(NULL == pSecInfoHeadNode->pHeadAttrNode)
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, "Malloc for the head of pSecInfoHeadNode->pHeadAttrNode failed!\n");
        free(pSecInfoHeadNode);
        pSecInfoHeadNode = NULL;
		return NULL;
	}
	//These two are unuseable for pSecInfoHeadNode
	memset(pSecInfoHeadNode->sectionName, 0x00, MOUTILS_INI_SECTION_NAME_MAX_LEN);
	memset(pSecInfoHeadNode->pHeadAttrNode, 0x00, sizeof(MOUTILS_INI_ATTR_INFO_NODE));
 ...
 * */
static void UnInitSecInfoList(MOUTILS_INI_SECTION_INFO_NODE *pSecInfoHeadNode)
{
	MOUTILS_INI_SECTION_INFO_NODE *pCurSecInfoNode = pSecInfoHeadNode->next;
	while(pCurSecInfoNode != NULL)
	{
		MOUTILS_INI_ATTR_INFO_NODE *pCurAttrInfoNode = pCurSecInfoNode->pHeadAttrNode->next;
 ...
 *
 * */
static int AddSecInfoNode(const char *pSecName, const MOUTILS_INI_SECTION_INFO_NODE *pSecInfoHeadNode)
{
	if(NULL == pSecName)
	{
	...
	if(NULL == pSecInfoHeadNode)
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, "pSecInfoHeadNode is NULL! Do init firstly before adding section node!\n");
		return -2;
	}

	int isNodeExist = -1;
	MOUTILS_INI_SECTION_INFO_NODE *pLastSecInfoNode = (MOUTILS_INI_SECTION_INFO_NODE *)pSecInfoHeadNode;
	...
	if(0 == isNodeExist)
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, "Input section node (sectionname=[%s]) has been exist in section list!"
				"Cannot being add again!\n", pSecName);
		return -3;
	}
	//Malloc memory for this new section node
	MOUTILS_INI_SECTION_INFO_NODE *pNewSecInfoNode = NULL;
	pNewSecInfoNode = (MOUTILS_INI_SECTION_INFO_NODE *)malloc(sizeof(MOUTILS_INI_SECTION_INFO_NODE));
	if(NULL == pNewSecInfoNode)
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, 
            "Malloc for the new section(name=[%s]) failed!" \
			"Cannot being added!\n", pSecName);
		return -4;
	}
	pNewSecInfoNode->pHeadAttrNode = (MOUTILS_INI_ATTR_INFO_NODE *)malloc(sizeof(MOUTILS_INI_ATTR_INFO_NODE));
	if(NULL == pNewSecInfoNode->pHeadAttrNode)
	{
        free(pNewSecInfoNode);
        pNewSecInfoNode = NULL;
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, 
            "Malloc for the head of section(name=[%s]) failed!" \
			"Cannot being added!\n", pSecName);
		return -5;
	}

	pNewSecInfoNode->next = NULL;
	strncpy(pNewSecInfoNode->sectionName, pSecName, MOUTILS_INI_SECTION_NAME_MAX_LEN);
	memset(pNewSecInfoNode->pHeadAttrNode, 0x00, sizeof(MOUTILS_INI_ATTR_INFO_NODE));
 ...
 * */
static int AddAttrInfoNode(const MOUTILS_INI_ATTR_INFO *pAttrInfo, const char *pSecName, const MOUTILS_INI_SECTION_INFO_NODE *pSecInfoHeadNode)
{
	...
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, 
            "pSecInfoHeadNode is NULL, cannot add attribute to any section!\n");
		return -2;
	}

	//Depend on the pSecName, find the section node in global section node list
	int isFindSection = -1;
	MOUTILS_INI_SECTION_INFO_NODE *pTargetSecNode = pSecInfoHeadNode->next;
	...
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, 
            "The section(name=[%s]) donot being found in gSecInfoNodeList!"
			"Cannot add attribute to it surely.\n", pSecName);
		return -3;
	}
	//Add input attribute node to the section being found
	int isFindAttr = -1;
	MOUTILS_INI_ATTR_INFO_NODE *pLastAttrInfoNode = pTargetSecNode->pHeadAttrNode;
	...
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, 
            "Attr node has been exist in section [%s], cannot add it again!\n",
			pSecName);
		return -4;
	}
	//Malloc a new node for the attribute
	MOUTILS_INI_ATTR_INFO_NODE *pNewAttrInfoNode = NULL;
	pNewAttrInfoNode = (MOUTILS_INI_ATTR_INFO_NODE *)malloc(sizeof(MOUTILS_INI_ATTR_INFO_NODE));
	if(NULL == pNewAttrInfoNode)
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, "Malloc for new attribute failed!\n");
		return -5;
	}
	memcpy(&pNewAttrInfoNode->attrInfo, pAttrInfo, sizeof(MOUTILS_INI_ATTR_INFO));
...
static int GetFirstValidCharType(FIRST_VALID_CHAR_TYPE *type, const char *line)
{
	if(NULL == line)
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, "Input line is NULL!\n");
		return -1;
	}
	int ret = 0;
...
static int GetSectionNameFromLine(char *sectionName, char *line, const unsigned int firstValidCharPos)
{
	...
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, 
            "line [%s] donot find symbol ], not a right format section line.\n", line);
		return -2;
	}
	int i = 1;
	for(; *(lastRightBracketPos + i) != 0x00; i++)
	{
		...
		{
			moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
                "In line [%s], after ], valid charactor [%c] appeared! Its not a right format section!",
	...
	if(strlen(tempSectionName) >= MOUTILS_INI_SECTION_NAME_MAX_LEN)
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
            "line [%s] has section [%s], its length [%d], larger than MOUTILS_INI_SECTION_NAME_MAX_LEN(%d)\n",
			line, tempSectionName, strlen(tempSectionName), MOUTILS_INI_SECTION_NAME_MAX_LEN);
		return -3;
	}
...
static int GetAttrFromLine(char *key, char *value, char *line, const unsigned int firstValidCharPos)
{
	...
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
            "In line [%s], no valid equal symbol appeared, not an attribute line!\n", 
            line);
		return -2;
	}
	...
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
            "In line [%s], more than 1 equal appeared, not an attribute line!\n", 
            line);
		return -3;
	}
	...
	if(tempKeyLen <= 0 || tempKeyLen >= MOUTILS_INI_ATTR_KEY_MAX_LEN)
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
            "In line [%s], Key = [%s], length(%d) less than ATTR_KEY_MIN_LEN(1) or "
			"larger than MOUTILS_INI_ATTR_KEY_MAX_LEN(%d)!\n",
			line, tempKey, tempKeyLen, MOUTILS_INI_ATTR_KEY_MAX_LEN);
		return -4;
	}
	tempValueLen = strlen(tempValue);
	if(tempValueLen <= 0 || tempValueLen >= MOUTILS_INI_ATTR_VALUE_MAX_LEN)
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
            "In line [%s], Value = [%s], length(%d) less than ATTR_KEY_MIN_LEN(1) or "
			" larger than MOUTILS_INI_ATTR_VALUE_MAX_LEN(%d)!\n",
			line, tempValue, tempValueLen, MOUTILS_INI_ATTR_VALUE_MAX_LEN);
		return -5;
	}
	//Get key and value
	strcpy(key, tempKey);
 ...
 * 			2.1.1.length in [1, MOUTILS_INI_SECTION_NAME_MAX_LEN] : section line, return LINE_SECTION;
 * 			2.1.2.length not in [1, MOUTILS_INI_SECTION_NAME_MAX_LEN] : error line, return LINE_ERROR;
 * 		2.2.not get it : error line, return LINE_ERROR;
 * 3.Get '=' in this line:
 * 		3.1.get key and value from it:
 * 			3.1.1.key_length in [1, MOUTILS_INI_ATTR_KEY_MAX_LEN], and value length in [1, MOUTILS_INI_ATTR_VALUE_MAX_LEN] : attr line, return LINE_ATTR;
...
static LINE_TYPE GetLineType(char *sectionName, char *key, char *value, char *line)
{
	...
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
            "GetFirstValidCharType failed, ret = %d.\n", firstValidCharPos);
	}
	...
	else if(firstValidCharType == TYPE_LEFT_BRACKET)	//section
	{
		...
		{
			moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
                "GetSectionNameFromLine failed, ret = %d, line = [%s].\n", ret, line);
			type = LINE_ERROR;
		}
	}
	else if(firstValidCharType == TYPE_OTHER_VALID_CHAR)	//attr
	{
		...
		{
			moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
                "GetAttrFromLine failed, ret = %d, line = [%s].\n", ret, line);
			type = LINE_ERROR;
		}
	}
 ...
 * 5.loop to step1;
 * */
static int GetSectionList(FILE *fp, MOUTILS_INI_SECTION_INFO_NODE *pSecInfoHeadNode)
{
	if(NULL == fp)
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, "Input param is NULL!\n");
	...
	if(NULL == pSecInfoHeadNode)
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, "pSecInfoHeadNode is NULL! Do init firstly!\n");
		return -2;
	}
	...
	char curSectionName[MOUTILS_INI_SECTION_NAME_MAX_LEN];
	memset(curSectionName, 0x00, MOUTILS_INI_SECTION_NAME_MAX_LEN);
	while(NULL != fgets(line, MAX_LINE_LEN, fp))
	{
		...
		char sectionName[MOUTILS_INI_SECTION_NAME_MAX_LEN], key[MOUTILS_INI_ATTR_KEY_MAX_LEN], value[MOUTILS_INI_ATTR_VALUE_MAX_LEN];
		...
		case LINE_SECTION:
		{
			...
			if(0 != ret)
			{
				moLoggerError(MOUTILS_LOGGER_MODULE_NAME, "AddSecInfoNode for section [%s] failed!\n", sectionName);
				ret = -1;
				break;
			}
		...
		case LINE_ATTR:
		{
			...
			{
				moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
                    "Find attribute which donot being contained in a section! "
			...
			{
				moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
                    "AddAttrInfoNode(attrKey=[%s], attrValue=[%s]) for section [%s] failed!\n",
		...
		{
 			moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
                "Parse line [%s] error, it is not a valid config file.\n", line);
			ret = -4;
			break;
		}
		case LINE_COMMENT:
...
MOUTILS_INI_SECTION_INFO_NODE * moUtils_Ini_Init(const char *filepath)
{
	if(NULL == filepath)
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, "Input filepath is NULL!\n");
		return NULL;
	}
	...
	if(NULL == pHeadNode)
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, "InitSecInfoList failed!\n");
		return NULL;;
	}
	...
	if(NULL == fp)
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, "Open file [%s] failed!\n", filepath);
	...
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME, 
            "GetSectionList from [%s] failed!, ret = %d\n", filepath, ret);
...
static void DumpSectionInfo(const MOUTILS_INI_SECTION_INFO_NODE *pSecInfoNode)
{
	printf("Section : [%s]\n", pSecInfoNode->sectionName);
	MOUTILS_INI_ATTR_INFO_NODE *pCurAttrInfoNode = pSecInfoNode->pHeadAttrNode->next;
...
void moUtils_Ini_DumpAllInfo(const MOUTILS_INI_SECTION_INFO_NODE *pSecInfoHeadNode)
{
	...
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
            "Should do init firstly, or cannot dump anything!\n");
		return ;
	}
        ...
        const MOUTILS_INI_SECTION_INFO_NODE *pSecInfoHeadNode)
{
	...
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
            "Do not init config file, cannot get attr info!\n");
		return -1;
	}
	...
	int isFindSec = -1;
	MOUTILS_INI_SECTION_INFO_NODE *pCurSecNode = pSecInfoHeadNode->next;
	...
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
            "Donot find input section [%s], cannot get any attribute infomation!\n",
			pSecName);
		return -3;
	}
	int isFindAttr = -1;
	MOUTILS_INI_ATTR_INFO_NODE *pCurAttrNode = pCurSecNode->pHeadAttrNode->next;
	...
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
            "Donot find input attr key [%s](section=[%s]), cannot get value for it!\n",
			pAttrKey, pSecName);
		return -4;
	}
...
int moUtils_Ini_UnInit(MOUTILS_INI_SECTION_INFO_NODE *pSecInfoHeadNode)
{
	...
	{
		moLoggerError(MOUTILS_LOGGER_MODULE_NAME,
            "Do not init config file, cannot do uninit operation!\n");
		return -1;
	}
